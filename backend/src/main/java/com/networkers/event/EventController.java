...
public class EventController {
    ...
    @GetMapping("/api/events/rsvps/mine") public ApiResponse<java.util.Map<Long,String>> myRsvps(){User user=CurrentUser.get();java.util.Map<Long,String> result=new java.util.HashMap<>();rsvps.findByUser(user).forEach(r->result.put(r.getEvent().getId(),r.getStatus().name()));return ApiResponse.ok("My event responses",result);}
