  ...
  return <AuthFrame title="Create your account" footer={<Link to="/login" className="font-semibold text-red-600">Already registered?</Link>}>
    <form onSubmit={submit} className="space-y-4">
      {["fullName", "email", "mobile", "password"].map((key) => (
        <input key={key} className="field" type={key === "password" ? "password" : "text"} placeholder={key === "fullName" ? "Full name" : key[0].toUpperCase() + key.slice(1)} value={form[key]} onChange={(e) => setForm({ ...form, [key]: e.target.value })} />
      ))}
