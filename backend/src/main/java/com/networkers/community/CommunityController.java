...
public class CommunityController {
    ...
    @GetMapping @Transactional(readOnly=true) public ApiResponse<List<PostView>> all(){List<Post> feed=posts.findAllByOrderByCreatedAtDesc();Map<Long,List<CommentView>> commentsByPost=new HashMap<>();if(!feed.isEmpty())for(Comment c:comments.findByPostInOrderByCreatedAtAsc(feed))commentsByPost.computeIfAbsent(c.getPost().getId(),ignored->new ArrayList<>()).add(commentView(c));return ApiResponse.ok("Community feed",feed.stream().map(p->view(p,commentsByPost.getOrDefault(p.getId(),List.of()))).toList());}
