...
export default function CreateUser() {
  ...
  useEffect(() => {
    ...
    if (visitor) setForm(current => ({ ...current, fullName: visitor.fullName || "", email: visitor.email || "", mobile: normalizePhone(visitor.mobile || ""), businessName: visitor.businessName || "", location: visitor.city || "" }));
  }, [location.state]);
  ...
  return (
    <div className="space-y-6"><form onSubmit={submit} className="card p-6">
      ...
      <h2 className="mt-1 page-title">Create <span className="text-[#E8262A]">Member</span></h2>
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-3">
        ...
        ].map(([key, label, type = "text"]) => <label key={key} className="grid gap-1"><span className="text-xs font-bold text-slate-500">{label}{["fullName","email","mobile","dateOfBirth"].includes(key)?" *":""}</span><input name={key} aria-label={label} type={type} max={key==="dateOfBirth"?new Date().toISOString().slice(0,10):undefined} min={key==="subscriptionEndDate"?form.subscriptionStartDate||undefined:undefined} required={["fullName", "email", "mobile", "dateOfBirth"].includes(key)} className="field" placeholder={type==="date"?undefined:label} value={form[key]} onChange={({ currentTarget: { value } }) => setForm((current) => ({ ...current, [key]: key === "mobile" ? normalizePhone(value) : value }))} /></label>)}
      ...
      <button disabled={chapters.length === 0 || creating || created} className="btn-primary mt-5">{creating ? "Creating member..." : "Create Member"}</button>
      {created && <div className="mt-5 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-5"><p className="text-lg font-black text-emerald-400">Member created successfully</p><p className="mt-1 text-sm text-slate-400">{created.user.fullName} · {created.user.email}</p><p className="mt-3 text-sm text-slate-400">The account is ready. Sending credentials is a separate action and can be retried if the mail server is unavailable.</p><div className="mt-4 flex flex-wrap gap-2"><button type="button" disabled={sending} className="btn-primary" onClick={sendCredentials}>{sending ? "Sending credentials..." : "Send credentials"}</button><button type="button" className="btn-muted" onClick={createAnother}>Create another member</button></div></div>}
