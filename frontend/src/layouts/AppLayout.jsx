...
import { BellRing, Cake, CreditCard, Medal, X } from "lucide-react";
...
export default function AppLayout() {
  ...
  useEffect(()=>{if(isAdmin||!user)return;notificationApi.all().then(items=>{const cycle=(items||[]).find(n=>!n.read&&n.title?.toLowerCase().includes("cycle")&&n.title?.toLowerCase().includes("completed"));if(cycle)addAlerts([{id:`cycle_${cycle.id}`,icon:Medal,title:cycle.title,message:cycle.message,to:"/meetings",key:`cycle_alert_${cycle.id}`}])}).catch(()=>{})},[user?.id,isAdmin]);
