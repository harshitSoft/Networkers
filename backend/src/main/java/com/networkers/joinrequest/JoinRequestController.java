...
public class JoinRequestController {
    private final JoinRequestRepository requests; private final UserRepository users;
    public JoinRequestController(JoinRequestRepository requests,UserRepository users){this.requests=requests;this.users=users;}
    ...
    private JoinRequest get(Long id){return requests.findById(id).orElseThrow(()->new jakarta.persistence.EntityNotFoundException("Join request not found"));}
    public record Request(@NotBlank String fullName,@Email @NotBlank String email,@NotBlank String mobile,String businessName,String businessCategory,String location,String message){}
}
