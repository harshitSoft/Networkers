...
export default function ManageUsers() {
  ...
  function startEdit(user) {
    setEditTarget({ fullName: user.fullName || "", email: user.email || "", mobile: normalizePhone(user.mobile), dateOfBirth: user.dateOfBirth || "", businessName: user.businessName || "", businessCategory: user.businessCategory || "", services: user.services || "", location: user.location || "", chapterId: String(user.chapter?.id || user.chapterId || ""), role: user.role || "USER", enabled: user.enabled !== false, subscriptionStartDate: user.subscriptionStartDate || "", subscriptionEndDate: user.subscriptionEndDate || "", password: "", id: user.id });
  }
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4" onClick={() => !saving && setEditTarget(null)}>
          <form className="card max-h-[90vh] w-full max-w-3xl overflow-y-auto p-6" onSubmit={saveEdit} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-2">
              {[["fullName","Full name","text"],["email","Email","email"],["mobile","Mobile","tel"],["dateOfBirth","Date of birth","date"],["businessName","Business name","text"],["businessCategory","Business category/work","text"],["services","Services","text"],["location","Location","text"],["subscriptionStartDate","Subscription start date","date"],["subscriptionEndDate","Subscription end date","date"]].map(([key,label,type]) => <label key={key}><span className="mb-1 block text-xs font-bold uppercase text-slate-500">{label}</span><input name={key} className="field" type={type} max={key === "dateOfBirth" ? new Date().toISOString().slice(0, 10) : undefined} required={["fullName","email","mobile"].includes(key)} value={editTarget[key]} onChange={({ currentTarget: { value } }) => setEditTarget((current) => ({ ...current, [key]: key === "mobile" ? normalizePhone(value) : value }))} /></label>)}
