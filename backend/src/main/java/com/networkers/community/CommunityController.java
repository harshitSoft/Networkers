...
public class CommunityController {
    ...
    private final MeetingParticipantRepository participants; private final UserRepository users; private final CloudinaryImageService media; private final NotificationService notifications;
    public CommunityController(PostRepository p,CommentRepository c,MonthlyMeetingRepository m,MeetingParticipantRepository participants,UserRepository u,CloudinaryImageService media,NotificationService n){posts=p;comments=c;meetings=m;this.participants=participants;users=u;this.media=media;notifications=n;}

    @PostMapping(consumes="multipart/form-data") @Transactional
    public ApiResponse<PostView> create(@RequestParam(required=false) Long meetingId,@RequestParam String caption,@RequestParam(required=false) List<Long> mentionIds,@RequestPart(value="file",required=false) MultipartFile file) throws IOException {
    ...
    private PostView view(Post p){User current=CurrentUser.get();List<CommentView> cs=comments.findByPostOrderByCreatedAtAsc(p).stream().map(this::commentView).toList();return new PostView(p.getId(),new MemberView(p.getUser().getId(),p.getUser().getFullName(),p.getUser().getProfileImage(),p.getUser().getBusinessName()),p.getContent(),p.getMediaUrl(),p.getMediaType(),p.getMeeting()==null?null:p.getMeeting().getId(),p.getMeeting()==null?null:p.getMeeting().getGroup().getChapter().getChapterName(),p.getMentions().stream().map(u->new MemberView(u.getId(),u.getFullName(),u.getProfileImage(),u.getBusinessName())).toList(),p.getKudos().size(),p.getKudos().stream().anyMatch(u->u.getId().equals(current.getId())),cs,p.getCreatedAt());}
    ...
    public record CommentRequest(String content){} public record UpdatePostRequest(String caption){} public record MemberView(Long id,String name,String avatar,String businessName){} public record CommentView(Long id,MemberView author,String content,java.time.LocalDateTime createdAt){} public record PostView(Long id,MemberView author,String caption,String mediaUrl,String mediaType,Long meetingId,String chapterName,List<MemberView> mentions,int kudosCount,boolean kudosGiven,List<CommentView> comments,java.time.LocalDateTime createdAt){}
}
